...
void SortParticles(){
	// Maby use TimSort for this one
	std::sort(&ParticlesContainer[0], &ParticlesContainer[MaxParticles]);
}


GLuint MakeComputeShader(){

	auto vertex_file_path = "Particle.computeshader";
	...
	glShaderSource(mComputeShader, 1, &FuckingCString, NULL);
...
int main(void)
{
	...
	for (float x = -size; x < size; x += step){
		for (float y = -size; y < size; y += step){
			for (float z = -size; z < size; z += step){
				...
				ParticlesContainer[FindUnusedParticle()] = p;
			}
		}
	}
	...
	do
	{
		...
		//  	ParticlesContainer[particleIndex].pos = glm::vec3(rand() % 10, rand() % 10, -30.0f);
		//  
		...
		for (int i = 0; i<MaxParticles; i++){
			...
			if (p.life > 0.0f){
				...
				if (p.life > 0.0f){
					...
					//p.velocity += glm::vec3(0.0f, -9.81f * (float)delta * 0.1f, 0.0f);
